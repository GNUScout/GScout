# .. coding: utf8
# $Id: __init__.py 7311 2012-01-10 02:33:33Z al-quaknaa $
# Author: Engelbert Gruber <grubert@users.sourceforge.net>
# Copyright: This module has been placed in the public domain.

"""ConTeXt document tree Writer."""
...
class Writer(writers.Writer):
    ...
    config_section = 'context writer'
    config_section_dependencies = ('writers',)

    head_parts = ('head_prefix', 'requirements', 'context_preamble', 'latex_preamble', 
    ...
    def assemble_parts(self):
        ...
        for part in self.visitor_attributes:
            ...
            if part in self.head_parts and not 'requirements':
                if lines:
...
# Building blocks for the context preamble
# --------------------------------------
...
% abstract title
\providecommand*{\DUtitleabstract}[1]{\centerline{{\bf #1}}}"""
...
class DocumentClass(object):
    ...
    def __init__(self, document_class, with_part=False):
        ...
        self.sections_numbered = ['part', 'chapter', 'section', 'subsection',
                                  'subsubsection', 'subsubsubsection',
                                  'subsubsubsubsection']
        self.sections_unnumbered = ['title', 'subject', 'subsubject',
                                    'subsubsubject',
                                    'subsubsubsubject']
        if self.document_class in ('book', 'memoir', 'report',
                                   'scrbook', 'scrreprt'):
            self.sections_numbered.insert(0, 'chapter')
        if self._with_part:
            self.sections_numbered.insert(0, 'part')
...
class LaTeXTranslator(nodes.NodeVisitor):
    ...
    def __init__(self, document, babel_class=Babel):
        ...
        self.latex_preamble = []
        self.context_preamble = [settings.context_preamble]
        self.stylesheet = []
    ...
    def visit_enumerated_list(self, node):
        ...
        #self.out.append('\\startitemize[%s]{%s\\%s{%s}%s}\n' %
        #                (enum_prefix,enum_type,counter_name,enum_suffix))
        self.out.append('\\startitemize[%s]\n' %
                    (enum_type))
    ...
    def visit_problematic(self, node):
        ...
        self.out.append('%\n')
        self.append_hypertargets(node)
        self.out.append(r'\hyperlink{%s}{{\bf \color{red}' % node['refid'])
    ...
    def visit_reference(self, node):
        ...
        if 'refuri' in node:
            ...
            if node['refuri'] == node.astext():
                ...
                self.out.append(r'\goto{%s}[url(%s)]' % (href, href))
                raise nodes.SkipNode
